...
public:
	...
	virtual void ray_trace_gpu(uint32_t n_elements, vec3* gpu_positions, vec3* gpu_directions, vec3* gpu_normals, float* gpu_depth, size_t* gpu_obj_id, size_t this_obj_id, const Triangle* gpu_triangles, cudaStream_t stream) = 0;
	virtual void shade_gpu(uint32_t n_elements, vec3* gpu_positions, vec3* gpu_views, vec3* gpu_normals, size_t* gpu_obj_ids, Triangle** gpu_objs, TriangleBvhNode** gpu_bvhs, sng::Material* gpu_materials, sng::Light sun, vec4* rgba, 
		float* depth, size_t this_obj_id, size_t vo_count, cudaStream_t stream) = 0;
