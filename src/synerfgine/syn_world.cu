...
__global__ void debug_reset_depth_syn(const uint32_t n_elements, float* __restrict__ depth);
...
void SyntheticWorld::reset_frame(CudaDevice& device, const ivec2& resolution) {
    ...
    if (resolution != m_resolution) {
        ...
        linear_kernel(debug_reset_depth_syn, 0, stream, n_elements, m_render_buffer_view.depth_buffer);
        CUDA_CHECK_THROW(cudaStreamSynchronize(stream));
    }
}

void SyntheticWorld::debug_init_rays(CudaDevice& device, const ivec2& resolution) {
    ...
    linear_kernel(init_buffer, 0, stream, n_elements, vec3(0.0), -0.1f, m_render_buffer_view.frame_buffer, m_render_buffer_view.depth_buffer);
    CUDA_CHECK_THROW(cudaStreamSynchronize(stream));
